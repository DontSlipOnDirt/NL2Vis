from core.utils import parse_response, validate_select_order, add_group_by, parse_vql_from_string, extract_world_info, is_email, is_valid_date_column
...
class Provider(BaseAgent):
    ...
    def talk(self, message: dict):
        ...
        query, tables = message.get('query'), message.get('tables')
        ...
        try:
            result = self._process(query=query, db_schema=db_schema)
        except Exception as e:
            print(e)
            result = {
                "filtered_schema": {},
...
"""
    ...
    def _translate(self, vql: str, library="matplotlib"):
        try:
            ...
            if vis_type in ["STACKED BAR", "GROUPED LINE", "GROUPED SCATTER"] or len(select_columns) == 3:
                return self._translate_plus(vql, library)
            else:
                return self._translate_normal(vql, library)
        except Exception as e:
    ...
    def _is_need_refine(self, exec_result: dict): 
        ...
        if exec_result['error']:
            ...
            if "FutureWarning" in exec_result['error']:
                flag = False
            if "UserWarning: Starting a Matplot" in exec_result['error']:
                flag = False
        return flag

    def _refine_vql(self, nl_query: str, vql: str, db_info, exec_result: dict, augmented_explanation):
    ...
    def talk(self, message: dict):
        if message['send_to'] != self.name: return
        self._message = message
        vql, query = message.get('final_vql'), message.get('query')
        db_info = message.get('new_schema')
        augmented_explanation = message.get('augmented_explanation')

        library = message.get('library', 'matplotlib')
        code = self._translate(vql, library)
        ...
        if is_timeout:
            ...
            new_vql = self._refine_vql(query, vql, db_info, exec_result, augmented_explanation)
            message['try_times'] = message.get('try_times', 0) + 1
